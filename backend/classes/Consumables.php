...
class Consumables {
    ...
    public function Add() {
        $connection = Connection::connect();
        $stmt = $connection->prepare("INSERT INTO `consumables` (name, description, date_received, image, quantity, responsible_user_id, temp_responsible_user_id, consumable_type_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssbiiii", $this->name, $this->description, $this->date_received, $this->image, $this->quantity, $this->responsible_user_id, $this->temp_responsible_user_id, $this->consumable_type_id);
    ...
    public function Update() {
        $connection = Connection::connect();
        $stmt = $connection->prepare("UPDATE `consumables` SET name = ?, description = ?, date_received = ?, image = ?, quantity = ?, responsible_user_id = ?, temp_responsible_user_id = ?, consumable_type_id = ? WHERE id = ?");
        $stmt->bind_param("sssbiiiii", $this->name, $this->description, $this->date_received, $this->image, $this->quantity, $this->responsible_user_id, $this->temp_responsible_user_id, $this->consumable_type_id, $this->id);
        $stmt->send_long_data(3, $this->image);
        $result = $stmt->execute();
        Connection::close($connection);
        return $result;
