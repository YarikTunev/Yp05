...
class Equipment {
    ...
    public function __construct($params)
    {
        ...
        if(isset($params["photo_path"]))$this->photo = $params['photo'] ?? ' ';
        if(isset($params["inventory_number"]))$this->inventory_number = $params['inventory_number'] ?? ' ';
        if(isset($params["room_id"]))$this->classroom_id = $params['classroom_id'] ?? ' ';
    ...
    public function Add()
    {
        ...
        $query = $connection->prepare("INSERT INTO `Equipment` (`name`, `photo`, `inventory_number`, `cost`, `direction_id`, `status_id`, `model_id`, `comment`, `classroom_id`, `responsible_user_id`, `temp_responsible_user_id`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public function Update()
    {
        ...
        $query = $connection->prepare("UPDATE `Equipment` SET `name`=?, `photo`=?, `inventory_number`=?, `cost`=?, `direction_id`=?, `status_id`=?, `model_id`=?, `comment`=?, `classroom_id`=?, `responsible_user_id`=?, `temp_responsible_user_id`=? WHERE `id`=?");
